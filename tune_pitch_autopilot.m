function res = tune_pitch_autopilot(params, K, sp)
    %% Initial conditions
    y = zeros(17,1);
    y(1:3) = [0; 0; -1];
    y(4:6) = [13; 0; 0];
    ...
    if isnan(info.RiseTime) || isnan(info.SettlingTime) || isnan(info.Overshoot)
        ...
        res.cost = res.cost + info.RiseTime + info.SettlingTime + abs(info.Overshoot - 4.598791)/100;
    end
end